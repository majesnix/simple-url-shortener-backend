...
class JanitorSpec extends AnyFlatSpec with Matchers {
  ...
  "removeInvalidUrls" should "remove URLs with disallowed schemes" in {
    val repo = makeRepo(Map("a" -> "https://example.com", "b" -> "ftp://files.example.com"))
    Janitor.removeInvalidUrls(repo).unsafeRunSync()
    repo.listAllUrls().unsafeRunSync().map(_._2).toSet shouldBe Set("https://example.com")
  }

  it should "remove URLs pointing at private IPs" in {
    val repo = makeRepo(Map("a" -> "https://example.com", "b" -> "http://192.168.1.1/admin"))
    Janitor.removeInvalidUrls(repo).unsafeRunSync()
    repo.listAllUrls().unsafeRunSync().map(_._2).toSet shouldBe Set("https://example.com")
  }

  it should "remove URLs with dangerous schemes" in {
    val repo = makeRepo(Map("a" -> "https://example.com", "b" -> "javascript:alert(1)"))
    Janitor.removeInvalidUrls(repo).unsafeRunSync()
    repo.listAllUrls().unsafeRunSync().map(_._2).toSet shouldBe Set("https://example.com")
  }

  it should "remove URLs pointing at loopback addresses" in {
    val repo = makeRepo(Map("a" -> "https://example.com", "b" -> "http://127.0.0.1:8080/path"))
    Janitor.removeInvalidUrls(repo).unsafeRunSync()
    repo.listAllUrls().unsafeRunSync().map(_._2).toSet shouldBe Set("https://example.com")
  }

  it should "not remove valid URLs" in {
    val repo = makeRepo(Map("a" -> "https://example.com", "b" -> "http://another.org/path?q=1"))
    Janitor.removeInvalidUrls(repo).unsafeRunSync()
    repo.listAllUrls().unsafeRunSync().map(_._2).toSet shouldBe Set("https://example.com", "http://another.org/path?q=1")
  }

  it should "handle an empty repository without error" in {
    val repo = makeRepo(Map.empty)
    noException should be thrownBy Janitor.removeInvalidUrls(repo).unsafeRunSync()
  }
}
